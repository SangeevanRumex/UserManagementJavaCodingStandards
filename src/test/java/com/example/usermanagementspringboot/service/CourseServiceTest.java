...
class CourseServiceTest {
    ...
    @Test
    void getCourseById() {
        Course requestedCourse = new Course(1,"Java",1000,
                                            false);
        CourseDto expectedCourse = new CourseDto(1,"Java",
                                                1000);

        Mockito.when(courseRepository.getCourseById(1))
                .thenReturn(requestedCourse);

        CourseDto responseCourse = courseServiceImpl.getCourseById(1);

        assertThat(responseCourse)
                .isEqualToComparingFieldByFieldRecursively(expectedCourse);
    }
}